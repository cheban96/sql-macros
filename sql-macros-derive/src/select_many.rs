...
fn generate_method(
    ...
    quote! {
        #[doc = #query]
        pub async fn #method_name(pool: &sqlx::PgPool, #params) -> Result<Vec<#struct_name>, sqlx::Error> {
...
pub fn expand(model: &TableModel) -> darling::Result<proc_macro2::TokenStream> {
    ...
    for column in model.columns.iter().filter(|c| c.is_select_many_filter) {
        for &op in &column.ops {
            let method_name = if op == model::Operator::Eq {
                format!("select_many_by_{}", column.ident)
            } else {
                format!("select_many_by_{}_{}", column.ident, model::op_word(op))
            };
    ...
    for custom in model.custom_select_many_methods()? {
